...
(function($) {
  ...
  $(function() {
    ...
    scroll_offset += $('#main-header').outerHeight();
     ...
     // Add this scroll event listener to adjust header opacity based on scroll
     $(window).on('scroll', function() {
      var header = $('#main-header');
      
      // If the user has scrolled more than 50px, make the header opaque
      if ($(this).scrollTop() > 50) {
        header.addClass('scrolled'); // Add the 'scrolled' class
      } else {
        header.removeClass('scrolled'); // Remove the 'scrolled' class when back at the top
      }
    });
